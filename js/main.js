 ...
 */
 	...
 	page_translate	: function(node){
		...
		/*cj add*/
		node[1].style[car2._prefixStyle('transform')] = 'translate(0,'+y_2+'px)';
 	...
 	page_touch_end	: function(e){
		...
		/* 不循环*/
		//if (!car2._pageNext && (car2._pageNext==0) && (car2._pageNow==(car2._pageNum-1))) {
		//	return;
		//}
 		
 		/*cj add*/
        if((car2._pageNow == car2._pageNum-1)&&(car2._pageNext == 0)) return;
 		...
 		if(Math.abs(car2._touchDeltaY)>10){
 			car2._page.eq(car2._pageNext)[0].style[car2._prefixStyle('transition')] = 'all 0.5s';
 			car2._page.eq(car2._pageNow)[0].style[car2._prefixStyle('transition')] = 'all 0.5s';
 		}
